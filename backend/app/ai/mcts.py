...
class MCTSEngine(AIEngine):
    ...
    # deadline_s 需考虑网关注入超时，前端代理超时已配置为 60s
    # 2026-08-19 加强：整体搜索深度 ×1.5~2.5，显著提升各档位 AI 强度
    DIFFICULTY_PRESETS: dict[str, tuple[int, int, float, float]] = {
        "beginner": (60,    60,  1.8, 2.0),   # 入门：适度加强，仍保留给新手空间
        "easy":     (220,   100, 1.5, 3.0),   # 初级：搜索深度 +120%
        "medium":   (900,   160, 1.4, 5.0),   # 中级：默认强度 ×2.25，接近原先 hard
        "hard":     (2800,  280, 1.2, 9.0),   # 高级：深度 +133%，时限放开到 9s
    }
    ...
    # 2026-08-19 二次升级（用户明确要求"背水一战失败才认输"）：
    #   - 胜率阈值再砍约一半，hard 难度近乎不认输（0.5% 胜率才考虑）
    #   - 最低落子数推到 80（19×19 棋盘 1/4 棋盘已填满）
    ...
    # 认输所需最低已落子数：40 → 80
    # 19×19 棋盘常见对局约 200-300 手；80 手约等于布局+中盘早期走完，
    # 此时大场基本落定，胜率信号足够可信，避免"背水一战"之前就认怂
